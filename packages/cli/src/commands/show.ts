import type { Finding, Verdict } from '@agent-conductor/core';
import { openContext } from '../context.js';
import { c, findingLine } from '../print.js';
...
export async function show(ref: string | undefined, flags: { json: boolean; advice: boolean }): Promise<number> {
  ...
  try {
    ...
    if (flags.json) return out(JSON.stringify({ run, task: task ? { ...task, spec_json: undefined, spec: JSON.parse(task.spec_json) as unknown } : null, rounds, workers, steps, verdicts }, null, 2)), 0;
    ...
    for (const r of rounds) {
      out(`\n${c.cyan(`round ${r.n}`)}`);
      for (const w of workers.filter((x) => x.round === r.n)) {
