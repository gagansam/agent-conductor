...
Nobody is watching this session. Do not ask questions and do not wait for approval: make the best decision you can, and record anything the operator should decide under `open_questions` in your report.
...
When you are done, write `{{output_path}}`: one JSON document that conforms to the JSON Schema in `{{pack_dir}}/OUTPUT-SCHEMA.json`. It must include `"schema_version": 1`, a `summary` of at most three sentences, and the status of every acceptance criterion by id. Write the file with your file-writing tool; do not only print it.
